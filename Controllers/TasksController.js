  ...
  try {
    ...
    if (!tasksData) {
      return res.status(400).json({
        success: false,
        message:
          "something went wrong while fetching task! or cannot find document",
      });
    }
    return res.status(200).json({
      success: true,
...
exports.patchTaskById = async (req, res, next) => {
  ...
  const patchableData = {
    ...
    completed:
      req.body.completed != null ? req.body.completed : taskData.completed,
  };
  try {
    const patchedData = await tasks.findByIdAndUpdate(
      taskID,
      { $set: { ...patchableData } },
      { new: true },
    );

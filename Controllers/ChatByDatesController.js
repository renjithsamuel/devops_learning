...
exports.getPreviousChats = async (req, res, next) => {
  ...
  try {
    ...
    const chatMessages = await chatByDates
...
exports.postChatByDate = async (req, res, next) => {
  if (
    req.body.text == null ||
    req.body.senderEmail == null ||
    req.body.senderName == null ||
    req.body.chatDate == null ||
...
exports.patchChatByDateById = async (req, res, next) => {
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Internal server error while fetching chatByDates",
      err,
    });
  }
  const patchableData = {
  ...
  try {
    const patchedData = await chatByDates.findByIdAndUpdate(
      chatByDateID,
      { $set: { ...patchableData } },
      { new: true },
    );
...
exports.deleteChatByDateById = async (req, res, next) => {
  ...
  try {
    const deletedChatByDateData =
      await chatByDates.findByIdAndDelete(chatByDateId);
...
exports.deleteManyChatsByCategoryId = async (req, res, next) => {
  ...
  try {
    ...
    const deletedChatsData = await chatByDates.deleteMany({
      category: categoryId,
    });

  ...
  try {
    ...
    if (!reportData) {
      return res.status(400).json({
        success: false,
        message:
          "something went wrong while fetching report! or cannot find document",
      });
    }
    return res.status(200).json({
      success: true,
...
exports.postReport = async (req, res, next) => {
  if (
    req.body.reportedDate == null ||
    req.body.reportStatement == null ||
    req.body.taskCompleted == null ||
    req.body.category == null ||
    req.body.reportedBy == null
  ) {
    return res.status(404).json({
      success: false,
...
exports.patchReportById = async (req, res, next) => {
  ...
  try {
    const patchedData = await reports.findByIdAndUpdate(
      ReportID,
      { $set: { ...patchableData } },
      { new: true },
    );
...
exports.deleteReportByTaskId = async (req, res, next) => {
  ...
  try {
    const checkReportWithTaskId = await reports.findOne({
    ...
    const deletedReportData = await reports.findOneAndDelete({
      taskCompleted: taskId,
    });
    if (!deletedReportData) {
      return res.status(400).json({
...
exports.deleteManyReportsByCategoryId = async (req, res, next) => {
  ...
  try {
    ...
    const deletedReportsData = await reports.deleteMany({
      category: categoryId,
    });

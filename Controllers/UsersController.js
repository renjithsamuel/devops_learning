...
exports.getUserByData = async (req, res, next) => {
  ...
  try {
    const currentUserData = await users
      .findOne({ username: req.body.username, emailId: req.body.emailId })
      .populate({
        path: "invites",
...
exports.getUserByEmail = async (req, res, next) => {
  ...
  try {
    const currentUserData = await users
      .findOne({ emailId: req.body.emailId }, { _id: 1 })
      .populate({
        path: "invites",
        populate: { path: "createdBy", model: "users", select: "username" },
      });
...
exports.patchUserById = async (req, res, next) => {
  ...
  if (
    ...
    req.body.invites == null &&
    req.body.updateInvite == null &&
    req.body.productivityPoints == null &&
  ...
  const patchableData = {
    ...
    invites:
      req.body.updateInvite == true || req.body.invites != null
        ? req.body.invites
        : userData.invites,
    productivityPoints:
      req.body.productivityPoints != null
        ? req.body.productivityPoints
        : userData.productivityPoints,
    streak: req.body.streak != null ? req.body.streak : userData.streak,
  };
  try {
    const patchedData = await users.findByIdAndUpdate(
      userID,
      { $set: { ...patchableData } },
      { new: true },
    );
...
exports.patchManyUserWithInvites = async (req, res, next) => {
  ...
  if (!Array.isArray(req.body.colaborators) || req.body.categoryId == null) {
    return res.status(400).json({
      success: false,
      error:
        "Send a valid response! Patching many requires an array of colaborators.",
    });
  }
  try {
    const data = await Promise.all(
      req.body.colaborators.map(async (colaboratorId) => {
        // Update colaborator with new invites
        const userInvites = await users.findOne(
          { _id: colaboratorId },
          { invites: 1 },
        );
